...
/*
 * Latched mouse-layer timeout selector.
 *
 * AML_TIME cycles through:
 *   0: 0 ms
 *   1: 500 ms
 *   2: 1000 ms
 *   3: 5000 ms
 *   4: 10000 ms
 *   5: never (stays active until a normal key dismisses it)
 *
 * Default is "never" to preserve the existing behavior.
 */
#define KEYBALL_MOUSE_TIMEOUT_NEVER 0xFFFFFFFFUL

static const uint32_t keyball_mouse_timeouts[] = {
    0,
    500,
    1000,
    5000,
    10000,
    KEYBALL_MOUSE_TIMEOUT_NEVER,
};

static uint8_t  keyball_mouse_timeout_index = 5;
static uint32_t keyball_mouse_last_motion    = 0;

static uint32_t keyball_get_mouse_layer_timeout(void) {
    return keyball_mouse_timeouts[keyball_mouse_timeout_index];
}


/*
 * Auto-mouse anti-jitter filter.
 *
 * Keyboard/desk vibration can move the PMW3360 by a few counts while typing.
 * Require sustained/meaningful motion before entering the mouse layer.
 */
#ifndef KEYBALL_AUTO_MOUSE_JITTER_THRESHOLD
#    define KEYBALL_AUTO_MOUSE_JITTER_THRESHOLD 8
#endif

#ifndef KEYBALL_AUTO_MOUSE_JITTER_WINDOW_MS
#    define KEYBALL_AUTO_MOUSE_JITTER_WINDOW_MS 50
#endif

#ifndef KEYBALL_AUTO_MOUSE_JITTER_MIN_REPORTS
#    define KEYBALL_AUTO_MOUSE_JITTER_MIN_REPORTS 2
#endif

static uint16_t keyball_auto_mouse_motion_accum   = 0;
static uint8_t  keyball_auto_mouse_motion_reports = 0;
static uint32_t keyball_auto_mouse_jitter_timer   = 0;

static uint16_t keyball_abs_mouse_xy(mouse_xy_report_t value) {
...
report_mouse_t pointing_device_task_combined_kb(report_mouse_t left_report, report_mouse_t right_report) {
    ...
    // Invert trackball scroll direction. Pointer X/Y movement is unchanged.
    output.v = -output.v;
    output.h = -output.h;
    ...
    /*
     * Auto mouse with anti-jitter filtering.
     *
     * Once the mouse layer is already active, any real pointer/scroll movement
     * refreshes its timeout immediately.
     *
     * When inactive, X/Y motion must:
     *   - accumulate at least KEYBALL_AUTO_MOUSE_JITTER_THRESHOLD counts,
     *   - happen within KEYBALL_AUTO_MOUSE_JITTER_WINDOW_MS, and
     *   - span at least KEYBALL_AUTO_MOUSE_JITTER_MIN_REPORTS reports.
     *
     * This prevents a single desk/keyboard vibration from activating MOUSE.
     * Scroll motion is treated as intentional and activates immediately.
     */
    const bool pointer_motion = output.x != 0 || output.y != 0;
    const bool scroll_motion  = output.h != 0 || output.v != 0;
    const uint32_t now        = timer_read32();

    if (layer_state_is(AUTO_MOUSE_DEFAULT_LAYER)) {
        if (pointer_motion || scroll_motion) {
            keyball_mouse_last_motion = now;
        }
    } else if (scroll_motion) {
        keyball_mouse_last_motion          = now;
        keyball_auto_mouse_motion_accum    = 0;
        keyball_auto_mouse_motion_reports  = 0;
        keyball_auto_mouse_jitter_timer    = now;
        layer_on(AUTO_MOUSE_DEFAULT_LAYER);
    } else if (pointer_motion) {
        if (timer_elapsed32(keyball_auto_mouse_jitter_timer) >
        ...
        keyball_auto_mouse_motion_accum +=
            keyball_abs_mouse_xy(output.x) + keyball_abs_mouse_xy(output.y);
        keyball_auto_mouse_motion_reports++;

        if (keyball_auto_mouse_motion_accum >= KEYBALL_AUTO_MOUSE_JITTER_THRESHOLD &&
            keyball_auto_mouse_motion_reports >= KEYBALL_AUTO_MOUSE_JITTER_MIN_REPORTS) {
            keyball_mouse_last_motion          = now;
            keyball_auto_mouse_motion_accum    = 0;
            keyball_auto_mouse_motion_reports  = 0;
            layer_on(AUTO_MOUSE_DEFAULT_LAYER);
...
#    ifdef AUTO_MOUSE_DEFAULT_LAYER
        uint32_t timeout = keyball_get_mouse_layer_timeout();

        if (layer_state_is(AUTO_MOUSE_DEFAULT_LAYER) &&
            timeout != KEYBALL_MOUSE_TIMEOUT_NEVER &&
            (timeout == 0 || timer_elapsed32(keyball_mouse_last_motion) >= timeout)) {
...
bool process_record_kb(uint16_t keycode, keyrecord_t *record) {
    ...
    case AML_TIME:
                ...
                // If 0 seconds is selected, dismiss the mouse layer immediately.
#    ifdef AUTO_MOUSE_DEFAULT_LAYER
                if (keyball_get_mouse_layer_timeout() == 0) {
                    layer_off(AUTO_MOUSE_DEFAULT_LAYER);
                }
#    endif
                break;
